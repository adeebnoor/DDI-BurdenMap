#!/usr/bin/env python3
"""Inpatient transport/sensitivity validation in the MIMIC-IV Clinical Database
Demo v2.2.

The hub watchlist is fixed before patient data are introduced. This script asks
whether that drug-level structure reaches real inpatient co-administration under
a stricter temporal-overlap definition. Candidate-network membership is a
prespecified structural target; it is not treated as a confirmed DDI or a fired
clinical alert.

The open demo is intentionally small (100 de-identified patients, 250 admissions),
so it is used as a secondary transport/sensitivity analysis rather than as a
second large definitive clinical cohort.
...
def main():
    ...
    ap.add_argument("--out", default="mimic_results.json")
    a = ap.parse_args()

    lookup = json.load(open(a.lookup)); G, cand_edges = load_candidate(a.candidate)
    deg = dict(G.degree()); N = len(deg)
    ranked = [d for d,_ in sorted(deg.items(), key=lambda kv: (-kv[1], kv[0]))]
    cand_nodes = set(deg)

    rx = pd.read_csv(a.prescriptions, low_memory=False)
    ...
    overlap_pairs = collections.Counter(); admission_pairs = collections.Counter(); n_adm = 0
    for _, g in rx_drug.groupby("hadm_id"):
        n_adm += 1; recs = []
        for _, r in g.iterrows():
            for d in r["db_ids"]: recs.append((d, r["starttime"], r["stoptime"]))
        drugs_here = {d for d,_,_ in recs}
            ...
            for j in range(i+1, len(ds)):
                ...
                for s1, e1 in by[ds[i]]:
                    for s2, e2 in by[ds[j]]:
                        ...
                        hi = min(e1 if pd.notna(e1) else pd.Timestamp.max,
                                 e2 if pd.notna(e2) else pd.Timestamp.max)
                        if pd.isna(lo) or lo <= hi: found = True; break
                    if found: break
                if found: overlap_pairs[(ds[i], ds[j])] += 1

    def restrict(counter):
        return {p:c for p,c in counter.items() if p[0] in cand_nodes and p[1] in cand_nodes}

    ov_in = restrict(overlap_pairs); ad_in = restrict(admission_pairs)
    ov_candidate = {p:c for p,c in ov_in.items() if p in cand_edges}
    ad_candidate = {p:c for p,c in ad_in.items() if p in cand_edges}
    ...
    def cover(counter, S, weighted=False):
        ...
        if weighted:
            ...
            return 100 * sum(c for (x,y),c in counter.items() if x in S or y in S) / den
        return 100 * sum(1 for x,y in counter if x in S or y in S) / len(counter)

    targets = [
        ("overlapping_pairs", ov_in),
        ("overlapping_candidate_network_pairs", ov_candidate),
        ("admission_pairs", ad_in),
        ("admission_candidate_network_pairs", ad_candidate),
    ]
    res = {
        "source": "MIMIC-IV Clinical Database Demo v2.2 (PhysioNet; ODbL v1.0; doi:10.13026/dp1f-ex47)",
        "prescription_rows": int(len(rx)), "rows_nondrug_excluded": int(tally["nondrug"]),
        "rows_unmapped": int(tally["unmapped"]), "rows_mapped": int(tally["mapped"]),
        "mapping_rate_pct_of_drug_rows": round(100*tally["mapped"] / max(tally["mapped"]+tally["unmapped"],1), 1),
        "admissions": n_adm, "subjects": int(rx_drug["subject_id"].nunique()),
        "unique_overlapping_pairs": len(overlap_pairs), "unique_admission_pairs": len(admission_pairs),
        "overlapping_pairs_in_candidate_network": len(ov_candidate),
        "admission_pairs_in_candidate_network": len(ad_candidate),
    ...
    for frac in (0.05, 0.10, 0.20):
        ...
        for label, P in targets:
            ...
            block[label] = {
                ...
                "covered_pct_event_weighted": round(obsw,1),
                "null_mean_pct": round(float(np.nanmean(null)),1),
                "null_95_pct": [round(float(q),1) for q in np.nanquantile(null,[.025,.975])],
                "enrichment_vs_null": round(obs/np.nanmean(null),2) if np.nanmean(null) else None,
