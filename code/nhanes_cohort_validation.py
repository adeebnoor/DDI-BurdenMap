...
people, what share of actually co-taken medication pairs involve a watchlist drug.
The all-pair endpoint is the primary external validation; the candidate-edge
subset is secondary because edge membership shares provenance with the ranking.
...
def main():
    ...
    dd_edges = set()
    if a.ddinter:
        dd = pd.read_csv(a.ddinter, dtype=str)
        for n1, n2 in zip(dd.get("Drug_A", []), dd.get("Drug_B", [])):
            d1, d2 = lookup.get(norm(n1)), lookup.get(norm(n2))
            if d1 and d2 and d1 != d2:
                dd_edges.add(tuple(sorted((d1, d2))))
        alertable_dd = {p for p in pairs if p in dd_edges}
    else:
        alertable_dd = set()
    ...
    res = {
        ...
        "pairs_in_ddinter": len(alertable_dd),
    ...
    if alertable_dd:
        targets.append(("ddinter_alertable_pairs", alertable_dd))
    ...
    for frac in (0.05, 0.10, 0.20):
        ...
        block["participants_exposed_to_watchlist_drug_pct"] = round(
            100 * exposed / len(per_person), 1)
    ...
    if a.demo:
        ...
        def describe(sub, label):
            ages = sub["RIDAGEYR"].dropna()
            sexes = sub["RIAGENDR"].dropna()
            return {
                "population": label,
                "n": int(len(sub)),
        ...
        res["cohort"] = {
            ...
            "with_any_prescription": describe(
                demo[demo["SEQN"].isin(with_rx)], "participants with >=1 mapped prescription"),
            "on_2plus_drugs": describe(
                demo[demo["SEQN"].isin(multi)], "participants on >=2 mapped drugs (contribute pairs)"),
        }
        n_med = int(demo["SEQN"].isin(with_rx).sum())
        res["cohort"]["note"] = (
            f"Coverage statistics are computed over the {n_med} participants with "
            "at least one mapped prescription; the all-participants row is shown "
            "only to document how the analytical cohort differs from the full survey sample.")
